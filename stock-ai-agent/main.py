...
async def run_migration():
    """執行 migration_v3.sql，新增外資持股、評分、新聞、回測等欄位與表格。"""
    import pathlib
    sql_path = pathlib.Path(__file__).parent.parent / "db" / "migration_v3.sql"
    if not sql_path.exists():
        return {"status": "error", "msg": "migration_v3.sql not found"}
