...
async def dashboard():
    ...
    pools, results, logs, stock_cnt = await asyncio.gather(
        ...
        fetch_all("""
            ...
            LIMIT 60
        """),
        fetch_all("SELECT l.*, p.pool_name FROM screener_logs l LEFT JOIN screener_pools p ON p.pool_id=l.pool_id ORDER BY started_at DESC LIMIT 10"),
        fetch_all("SELECT COUNT(*) AS n FROM screener_pool_stocks WHERE is_active=TRUE"),
    )

    screen_date = str(results[0]["screen_date"]) if results else "尚未執行月度篩選"
